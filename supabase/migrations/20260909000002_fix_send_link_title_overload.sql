-- 20260909000002_fix_send_link_title_overload.sql
-- Linkiac Migration: Retain original link titles, fix send_link_to_recipients overload, and remove 'Shared by @...' title overwriting

-- 1. Ensure sends.title column exists
alter table public.sends add column if not exists title text;

-- 2. Drop the 5-parameter overload to prevent PostgREST ambiguity (PGRST203)
drop function if exists public.send_link_to_recipients(text, text, uuid[], uuid, text);

-- 3. Create or replace the canonical 6-parameter broadcast send RPC
...
begin
  ...
  -- Determine title: priority given to passed title, then source link title, then domain/url
...
-- 4. Update accept_friend_suggestion RPC to retain original title (NEVER set 'Shared by @username')
...
begin
  ...
  -- Retain original title: send.title -> source_link.title -> domain -> url fallback (NEVER 'Shared by @...')
  v_title := coalesce(
  ...
  -- Insert into recipient library with genuine title
...
-- 5. Auto-heal any existing rows in public.links polluted with 'Shared by @...'
update public.links
set title = coalesce(nullif(trim(domain), ''), url)
where title ilike 'Shared by @%';
